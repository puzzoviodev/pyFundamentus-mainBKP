...
def evaluate_lpa(lpa):
    ...
    definicao = '''Indicador que mede o lucro líquido por ação, refletindo a lucratividade da empresa por ação emitida. É útil para avaliar a capacidade da empresa de gerar retorno aos acionistas.'''
...
def evaluate_psr(psr):
    ...
    definicao = '''Indicador que relaciona o preço da ação à receita líquida por ação, avaliando o valor de mercado em relação às vendas. É útil para empresas com lucros baixos ou negativos.'''
    agrupador = 'Valuation'
    ...
    try:
        if p_acl < 0:
            return {
                'classificacao': 'Critico',
                'faixa': 'P/ACL < 0',
                'descricao': 'P/ACL negativo indica ativo circulante líquido negativo, sugerindo problemas de liquidez ou passivo circulante maior que ativo circulante (ex.: OI).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Otimo',
                'faixa': '0 <= P/ACL <= 0.2',
                'descricao': 'P/ACL baixo sugere subvalorização em relação à liquidez, comum em mineração (ex.: VALE3). Oportunidade de compra, especialmente em setores com ativos circulantes robustos.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Moderado',
                'faixa': '0.2 < P/ACL <= 0.5',
                'descricao': 'P/ACL indica valuation justo, comum em energia (ex.: ELET3). Oferece equilíbrio entre risco e retorno.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Ruim',
                'faixa': '0.5 < P/ACL <= 1',
                'descricao': 'P/ACL elevado sugere sobrevalorização, comum em consumo (ex.: ABEV3). Exige análise detalhada de liquidez e endividamento.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Pessimo',
                'faixa': '1 < P/ACL <= 1.5',
                'descricao': 'P/ACL muito alto indica ação cara, comum em tecnologia (ex.: TOTS3). Risco de correção elevado devido a expectativas de mercado.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Fora da faixa',
                'faixa': 'P/ACL > 1.5',
                'descricao': 'P/ACL extremamente elevado sugere sobrevalorização severa, típica de empresas especulativas ou com baixa liquidez relativa (ex.: WEGE3). Análise detalhada é essencial.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
        ...
        return {
            'classificacao': 'Erro',
            'faixa': 'N/A',
            'descricao': f'Erro ao processar P/ACL: {str(e)}. Verifique se o valor fornecido é um número válido.',
            'definicao': definicao,
            'agrupador': agrupador,
            'formula': formula
        }
...
def evaluate_cobertura_juros(cobertura):
    ...
    definicao = '''Indicador que mede a capacidade da empresa de cobrir suas despesas com juros usando o EBIT. Valores altos indicam maior facilidade em pagar obrigações financeiras.'''
    agrupador = 'Endividamento'
...
def evaluate_receita_ativo_circulante(rac):
    ...
    definicao = '''Indicador que mede a eficiência da empresa em gerar receita a partir de seus ativos circulantes. Valores altos indicam maior eficiência no uso de ativos líquidos.'''
    agrupador = 'Eficiência'
    formula = 'RAC = Receita Líquida / Ativo Circulante'
    try:
        if rac < 0:
            return {
                'classificacao': 'Critico',
...
def evaluate_lucro_bruto_receita(lbr):
    ...
    definicao = '''Indicador que mede a margem bruta da empresa, mostrando a proporção da receita que permanece após os custos diretos. Valores altos indicam maior eficiência na gestão de custos operacionais.'''
    agrupador = 'Rentabilidade'
    formula = 'LBR = Lucro Bruto / Receita Líquida'
    try:
        if lbr < 0:
            return {
                'classificacao': 'Critico',
...
def evaluate_lucro_operacional_receita(lor):
    ...
    definicao = '''Indicador que mede a margem operacional da empresa, mostrando a proporção da receita que permanece após custos operacionais. Valores altos indicam eficiência operacional.'''
    agrupador = 'Rentabilidade'
    formula = 'LOR = Lucro Operacional / Receita Líquida'
    try:
        if lor < 0:
            return {
                'classificacao': 'Critico',
...
def evaluate_ebitda_receita(ebitda_r):
    ...
    definicao = '''Indicador que mede a margem EBITDA da empresa, mostrando a proporção da receita que permanece após custos operacionais, antes de juros, impostos, depreciação e amortização. Valores altos indicam forte geração de caixa operacional.'''
    agrupador = 'Rentabilidade'
    formula = 'EBITDA/R = EBITDA / Receita Líquida'
    try:
        if ebitda_r < 0:
            return {
                'classificacao': 'Critico',
...
def evaluate_preco_fcl(pfcl):
    ...
    definicao = '''Indicador que compara o preço da ação com o fluxo de caixa livre por ação, avaliando a capacidade da empresa de gerar caixa disponível após investimentos. Valores baixos indicam subvalorização.'''
    agrupador = 'Valuation'
    formula = 'P/FCL = Preço da Ação / Fluxo de Caixa Livre por Ação'
    try:
        if pfcl < 0:
            return {
                'classificacao': 'Critico',
...
def evaluate_valor_atual(valor_atual, vpa=None):
    ...
    definicao = '''Indicador que avalia o preço da ação em relação ao VPA ou isoladamente, indicando se a ação está subvalorizada ou sobrevalorizada em relação ao patrimônio.'''
    agrupador = 'Valuation'
    formula = 'Valor Atual/VPA = Preço da Ação / Valor Patrimonial por Ação'
    try:
        if valor_atual < 0:
            return {
                'classificacao': 'Critico',
        ...
        if vpa is None or vpa <= 0:
            ...
            if 0 <= valor_vpa <= 0.5:
                ...
                return {
                    'classificacao': 'Fora da faixa',
                    'faixa': 'Valor Atual/VPA > 2.0',
                    'descricao': 'Sobrevalorização severa em relação ao VPA (ex.: WEGE3).',
                    'definicao': definicao,
                    'agrupador': agrupador,
                    'formula': formula
                }
        ...
        return {
            'classificacao': 'Erro',
            'faixa': 'N/A',
            'descricao': f'Erro ao processar Valor Atual: {str(e)}. Verifique se o valor fornecido é um número válido.',
            'definicao': definicao,
            'agrupador': agrupador,
            'formula': formula
        }
...
def evaluate_patrimonio_liquido(pl):
    ...
    definicao = '''Indicador que representa o valor contábil da empresa pertencente aos acionistas, calculado como ativos totais menos passivos totais. Valores altos indicam maior solidez financeira.'''
    agrupador = 'Estrutura de Capital'
...
def evaluate_ativos(ativos):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa o total de ativos da empresa, incluindo circulantes e não circulantes. Valores altos indicam maior porte e capacidade de investimento.'''
    agrupador = 'Estrutura de Capital'
...
def evaluate_divida_bruta(divida_bruta):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa o total de obrigações financeiras da empresa, incluindo empréstimos e financiamentos de curto e longo prazo.'''
    agrupador = 'Endividamento'
...
def evaluate_disponibilidade(disponibilidade):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa o caixa e equivalentes de caixa da empresa, refletindo a liquidez imediata disponível para cobrir obrigações ou investir.'''
    agrupador = 'Liquidez'
...
def evaluate_divida_liquida(divida_liquida):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa a dívida bruta da empresa menos seu caixa e equivalentes, refletindo o endividamento real. Valores negativos indicam mais caixa que dívidas.'''
    agrupador = 'Endividamento'
...
def evaluate_valor_mercado(valor_mercado):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa o valor total de mercado da empresa, calculado pelo preço da ação multiplicado pelo número de ações. Reflete a percepção do mercado sobre o valor da empresa.'''
    agrupador = 'Valuation'
    formula = 'Valor de Mercado = Preço da Ação × Número de Ações'
    try:
        # Se for string, processar formato monetário brasileiro
        if isinstance(valor_mercado, str):
            # Remove "R$" e espaços
            valor_mercado = valor_mercado.replace('R$', '').replace(' ', '')
            # Substitui ponto (milhares) e vírgula (decimal)
            valor_mercado = valor_mercado.replace('.', '').replace(',', '.')
            valor_mercado = float(valor_mercado) / 1_000_000_000  # Converte para bilhões
        else:
            # Garante que é float
            valor_mercado = float(valor_mercado) / 1_000_000_000 if valor_mercado >= 1_000_000 else float(valor_mercado)

        if valor_mercado < 0:
            return {
                'classificacao': 'Critico',
                'faixa': 'Valor Mercado < 0',
                'descricao': 'Valor de mercado negativo, erro nos dados.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Pessimo',
                'faixa': '0 <= Valor Mercado <= 1',
                'descricao': 'Valor de mercado muito baixo, típico de small caps (ex.: ENAT3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Ruim',
                'faixa': '1 < Valor Mercado <= 5',
                'descricao': 'Valor de mercado limitado, comum em small caps (ex.: CYRE3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Moderado',
                'faixa': '5 < Valor Mercado <= 20',
                'descricao': 'Valor de mercado sólido, típico de mid caps (ex.: ABEV3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Otimo',
                'faixa': '20 < Valor Mercado <= 100',
                'descricao': 'Valor de mercado robusto, comum em large caps (ex.: ITUB4).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Fora da faixa',
                'faixa': 'Valor Mercado > 100',
                'descricao': 'Valor de mercado excepcional, típico de blue chips (ex.: VALE3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
        ...
        return {
            'classificacao': 'Erro',
            'faixa': 'N/A',
            'descricao': f'Erro ao processar Valor de Mercado: {str(e)}. Verifique se o valor é um número válido ou no formato R$ XXX.XXX.XXX.',
            'definicao': definicao,
            'agrupador': agrupador,
            'formula': formula
        }
...
def evaluate_valor_firma(valor_firma):
    ...
    Aceita valores no formato R$ 112.714.000 (converte para bilhões).'''
    definicao = '''Indicador que representa o valor total da empresa, incluindo o valor de mercado e a dívida líquida, refletindo o custo total para aquisição da empresa.'''
    agrupador = 'Valuation'
    formula = 'Valor de Firma = Valor de Mercado + Dívida Líquida'
    try:
        # Se for string, processar formato monetário brasileiro
        if isinstance(valor_firma, str):
            # Remove "R$" e espaços
            valor_firma = valor_firma.replace('R$', '').replace(' ', '')
            # Substitui ponto (milhares) e vírgula (decimal)
            valor_firma = valor_firma.replace('.', '').replace(',', '.')
            valor_firma = float(valor_firma) / 1_000_000_000  # Converte para bilhões
        else:
            # Garante que é float
            valor_firma = float(valor_firma) / 1_000_000_000 if valor_firma >= 1_000_000 else float(valor_firma)

        if valor_firma < 0:
            return {
                'classificacao': 'Critico',
                'faixa': 'Valor Firma < 0',
                'descricao': 'Valor de firma negativo, erro nos dados.',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Pessimo',
                'faixa': '0 <= Valor Firma <= 2',
                'descricao': 'Valor de firma muito baixo, típico de small caps (ex.: ENAT3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Ruim',
                'faixa': '2 < Valor Firma <= 10',
                'descricao': 'Valor de firma limitado, comum em small caps (ex.: CYRE3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Moderado',
                'faixa': '10 < Valor Firma <= 50',
                'descricao': 'Valor de firma sólido, típico de mid caps (ex.: ABEV3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Otimo',
                'faixa': '50 < Valor Firma <= 150',
                'descricao': 'Valor de firma robusto, comum em large caps (ex.: ITUB4).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
            ...
            return {
                'classificacao': 'Fora da faixa',
                'faixa': 'Valor Firma > 150',
                'descricao': 'Valor de firma excepcional, típico de blue chips (ex.: VALE3).',
                'definicao': definicao,
                'agrupador': agrupador,
                'formula': formula
            }
        ...
        return {
            'classificacao': 'Erro',
            'faixa': 'N/A',
            'descricao': f'Erro ao processar Valor de Firma: {str(e)}. Verifique se o valor é um número válido ou no formato R$ XXX.XXX.XXX.',
            'definicao': definicao,
            'agrupador': agrupador,
            'formula': formula
        }